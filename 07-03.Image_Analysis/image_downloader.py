...
def search_photo(keyword, offset=0, limit=100):
    # API 쿼리 조합
    keyword_enc = parse.quote_plus(keyword) # 검색 keyword
    q = "keyword={0}&offset={1}&limit={2}".format(keyword_enc, offset, limit)
    url = PHOTOZOU_API + "?" + q # API url
    # 캐시 전용 폴더 생성
    if not os.path.exists(CACHE_DIR): # 폴더가 없을 경우 생성
        os.makedirs(CACHE_DIR)
    cache = CACHE_DIR + "/" + re.sub(r'[^a-zA-Z0-9\%\#]+', '_', url) # re.sub() 문자열 치환
    if os.path.exists(cache):
        return json.load(open(cache, "r", encoding="utf-8"))
    print("[API] " + url)
    req.urlretrieve(url, cache) # url 데이터 다운로드
    time.sleep(1) # --- 서버 접근 간격 1초
    return json.load(open(cache, "r", encoding="utf-8"))
# 이미지 다운로드
def download_thumb(info, save_dir):
    if not os.path.exists(save_dir): os.makedirs(save_dir) # 저장 폴더가 없을 경우 생성
...
if __name__ == '__main__':
    # 모듈로 사용할 수 있게 설정
    download_all("dessert", "./image/dessert") # --- (※4)